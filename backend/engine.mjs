...
let ticks = 0;
let binanceWs = null; // Track Binance connection to avoid duplicates
...
async function checkArbitrage(spotPrice) {
    for (let i = activeMarkets.length - 1; i >= 0; i--) {
        ...
        try {
            ...
            const is404 = err.message?.includes('404') || err.response?.status === 404 || JSON.stringify(err).includes('404');
            
            if (is404) {
                console.log(chalk.bgRed.white(`\n> ⛔ MARKET EXPIRED: ${market.slug} `));
                broadcast('ERROR', { message: `Market Expired: ${market.slug}` });
