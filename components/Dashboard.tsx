...
export const Dashboard: React.FC = () => {
  ...
  // NEW: run equity (authoritative Gate-4 view)
  useEffect(() => { // CHANGED
    let stop = false; // CHANGED
    async function loadEquity() { // CHANGED
      try { // CHANGED
        if (!runId) { // CHANGED
          setRunEquity(null); // CHANGED
          return; // CHANGED
        } // CHANGED

        const { data, error } = await supabase // CHANGED
          .from("bot_run_equity_current") // CHANGED
          .select("*") // CHANGED
          .eq("run_id", runId) // CHANGED
          .maybeSingle(); // CHANGED

        if (stop) return; // CHANGED

        if (error) { // CHANGED
          setErrEquity(formatSbError("bot_run_equity_current", error)); // CHANGED
          return; // CHANGED
        } // CHANGED

        setErrEquity(null); // CHANGED
        setRunEquity((data as any) ?? null); // CHANGED
      } catch (e: any) { // CHANGED
        if (stop) return; // CHANGED
        setErrEquity( // CHANGED
          `bot_run_equity_current unexpected: ${String(e?.message ?? e)}` // CHANGED
        ); // CHANGED
      } // CHANGED
    } // CHANGED

    loadEquity(); // CHANGED
    const i = setInterval(loadEquity, 3000); // CHANGED
    return () => { // CHANGED
      stop = true; // CHANGED
      clearInterval(i); // CHANGED
    }; // CHANGED
  }, [runId]); // CHANGED
  ...
  // Net Equity comes from authoritative Gate-4 view (bot_run_equity_current)
  ...
  const banner = useMemo(() => {
    ...
    const any403 =
      ...
      (errEquity || "").includes("403"); // CHANGED
    ...
    if (errRun || errTicks || errBankroll || errSettlements || errValuations || errEquity) { // CHANGED
      return {
        kind: "warn" as const,
        text:
  ...
  }, [errRun, errBankroll, errSettlements, errTicks, errValuations, errEquity, runId]); // CHANGED
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200 p-6">
      ...
      <div className="grid grid-cols-5 gap-4 mb-8">
        ...
        <Metric label="Net Equity" value={runEquity ? runEquity.net_equity.toFixed(2) : "--"} /> {/* CHANGED */}
      </div>
      ...
      <div className="mt-8">
        ...
        {errEquity && <ErrorBox title="bot_run_equity_current error" text={errEquity} />} {/* CHANGED */}
